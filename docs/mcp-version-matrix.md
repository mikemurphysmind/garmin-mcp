...
transports exist, 154 tools and all five resources are registered, and the policy
...
| Client registration — preregistration | required | M2 | none | Operator-configured clients with exact redirect URIs by default, or one trailing-path wildcard pattern per client under the operator acknowledgement ADR 0009 describes. This is the default, and the only mode enabled in production by default. No vendor client ID is hardcoded. |
