...
| 9 | Explicit widget MFA code delivery (upstream GH-386): request the code explicitly instead of assuming that the page sends it. | widget login strategy | **landed.** `ClassifyWidgetLogin` parses the page's inline variables (`customerGuid`, `mfaMethod`, `locale`, `clientId`, `codeSentTo`) and `Authenticator.requestWidgetMFACode` POSTs `PathWidgetRequestMFACode` when the method is email or SMS and no code has been sent yet. The parsed method outranks the title guess, so an authenticator-app page is never asked for a delivery it cannot make, and a page naming where a code went is not asked again. A confirmed delivery clears `MFADeliveryUncertain`. One divergence, deliberate: a failed delivery request does not fail the login — upstream raises — because the account may have been sent a code by the sign-in POST anyway; the caller is told delivery is uncertain instead, and the request is never retried. |
...
All ten items are landed. Item 9, explicit widget MFA code delivery, was the last
one outstanding and closed on 2026-08-16.
