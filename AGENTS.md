...
- **Supported platforms are Linux and macOS, on amd64 and arm64. Windows is not
  supported, and that is deliberate — do not add it back.** There is no
  Windows-tagged file, no `windows-latest` job, no Windows release archive, and
  no `GOOS=windows` in the vet loop or the build matrix. `internal/securefile`
  therefore only compiles on unix, which is honest: the whole point of that
  package is refusing to hold a secret under permissions it cannot verify, so a
  platform it cannot enforce on should fail to build rather than ship a weaker
  guarantee. Removing the ACL layer while keeping a Windows binary is the one
  outcome to avoid, because owner-only enforcement would silently stop applying.
...
`golang.org/x/sys/unix` for the TTY password prompt. `modernc.org/libc` moves only together with
...
subcommand — and no MCP conformance job.

Two entries this list used to carry are gone because the things exist: there are
eight `Fuzz*` targets, which the `fuzz-smoke` job discovers from source and runs
bounded, and `docs/operations.md` is written, in eight sections. A "still
missing" list is where documentation rots first — nobody deletes a line from one
when the thing gets built — so check the repository before trusting an entry
here.
...
setting, `login-allowed-emails`, restricts which Garmin account addresses may
complete the remote browser login; empty (the default) admits any account, and
the setting gates login rather than an existing principal.

### Safety model [NOW]

- Read-only tools are always registered. On stdio, explicit operator enablement
  authorizes the write and destructive tiers. On streamable-http, those tiers
  need the **intersection** of operator enablement and granted OAuth scope.
- Stdio has no OAuth caller and reports no granted scopes. The composition root
  marks local operator authority separately; policy construction rejects that
  authority in remote mode or beside a scope source. Streamable-http takes scopes
...
| E2E | `go test -race -count=1 -tags=e2e -timeout=10m ./e2e/...` | `e2e` | stdio and Streamable HTTP MCP, OAuth flow, browser login form, tenant isolation | **[NOW]** over the real binary. `cli_test.go` covers version output, a clean stdout on the stdio path, an unknown command, and a protocol-level `tools/list` control proving enabled stdio write and destructive tiers appear while the same tools stay absent with both flags off. Every launched subprocess has its inherited `GARMIN_MCP_*` environment stripped before the test's own offline settings are added, so an operator's ambient `GARMIN_MCP_DATABASE_PATH` (which outranks a config file) cannot redirect a subprocess at the operator's real state. `exercisecatalog_test.go` points the process at a proxy that refuses every tunnel and proves the start-up catalog read is attempted, is anonymous, reaches only `connect.garmin.com:443`, and cannot fail a start-up; every other binary this suite starts is pointed at a blackhole proxy by default, so no e2e run reaches the public Garmin service. `remote_test.go` stands up a synthetic TLS deployment and covers protected resource metadata read unauthenticated with `bearer_methods_supported` exactly `["header"]`, an untokened MCP request refused with a challenge carrying `resource_metadata` and no error code, a token in a query parameter that never authenticates, and a bad header token reported as `invalid_token`. `oauthflow_test.go` drives the authorization-code grant against the real `/token` endpoint: PKCE S256 is required, proven with both a negative case (wrong or missing `code_verifier`) and a positive control (the correct verifier, a fresh code seeded through the production digest helper, succeeds) so the negative case cannot pass merely because `/token` always answers `invalid_grant`; the redirect URI must match the code's exactly, with the same positive-control shape; a code fired at with N concurrent redemptions mints exactly one token, which a serial replay cannot prove; a plain PKCE method is refused at `/authorize` itself; and a token this endpoint issues is accepted by the real MCP endpoint. `loginform_test.go` drives the remote browser login profile with an `http.Client`: the disclosure page and the `__Host-` cookie's full attribute set, the rotating CSRF token (wrong is refused, right advances the transaction, the now-rotated value is refused on replay, and one session's token under another session's cookie is refused), and a credential submission that fails safely — proven, not merely assumed, by pointing the deployment at a recording CONNECT proxy and asserting the attempt reached exactly `sso.garmin.com` with no `Authorization` or `Cookie` header, which a build that deleted the `authenticator.Login` call cannot pass. `tenantisolation_test.go` mints two independent principals' bearer tokens through the real `/token` endpoint, proves that revoking one principal's token never invalidates the other's live session, and further resolves each token through the store to its principal ID and requires the two IDs to differ — the property "tenant isolation" actually names, which the revocation check alone does not establish. That last assertion catches the reviewed mutant — verified by applying it and observing the failure — and passes against unmodified code. Getting there required fixing the harness rather than the server: seeding used to write to the database while the server process held it, which is two writers on one SQLite file and violates this project's own single-writer requirement. The server's own error named it, `disk I/O error (522)`, which is `SQLITE_IOERR_SHORT_READ`. It made rows the server had written unreadable from the test process, and made a token issued under concurrent redemption fail its own verification. Every seed, the client row included, now runs before the subprocess starts, and nothing writes to the database while it runs; the concurrency assertion that had to be weakened while this was misdiagnosed as a product defect is restored. One piece of the browser-login row remains out of reach at this layer and is not simulated: completing a login, including an MFA continuation, requires a real Garmin response. Redirecting the compiled binary's own Garmin traffic there is not impossible — `internal/cmd/components.go`'s transport honours `HTTPS_PROXY` and, on Linux, `SSL_CERT_FILE` — but it needs a TLS-terminating proxy that mints a certificate per host, which is expensive to build and silently unavailable on macOS; see `docs/implementation-status.md` for what the current recording-only proxy already proves instead. Completing consent in the OAuth-flow tests is seeded directly into the SQLite store instead of driven through that same unreachable login, with the exact bindings a granted consent would have produced; everything downstream of that point (PKCE, the exact redirect match, single-use, token issuance, the MCP call) runs over real HTTP against the real process |
...
- The container image builds, passes the non-root/read-only smoke test, reaches
  `/readyz` with a mounted `/data`, writes its database and key there owner-only,
  and fails promptly when `/data` is read-only. The image needs a binary in the
  build context; see ADR 0006 for why the `Dockerfile` is runtime-only. A remote
  deployment needs a real certificate even on loopback, because the authorization
  server refuses to name a cleartext issuer at any origin.
